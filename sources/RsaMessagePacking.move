module keyring::rsa_message_packing {
    ...
    public fun pack_auth_message(
        ...
        let addr_bytes_len = length(&addr_bytes);
        if (addr_bytes_len > 1) {
            ...
            while (i < bytes_to_copy) {
                let src_byte: u8 = *borrow(&addr_bytes, i + 1);
                let dst_ref = borrow_mut(&mut result, i);
                *dst_ref = src_byte;
                i = i + 1;
            };
        };
        ...
        push_back(&mut result, ((policy_id >> 16) & 0xFF) as u8);
        push_back(&mut result, ((policy_id >> 8) & 0xFF) as u8);
        push_back(&mut result, (policy_id & 0xFF) as u8);
        
        // Add valid_from (4 bytes = 32 bits)
        push_back(&mut result, ((valid_from >> 24) & 0xFF) as u8);
        push_back(&mut result, ((valid_from >> 16) & 0xFF) as u8);
        push_back(&mut result, ((valid_from >> 8) & 0xFF) as u8);
        push_back(&mut result, (valid_from & 0xFF) as u8);
        
        // Add valid_until (4 bytes = 32 bits)
        push_back(&mut result, ((valid_until >> 24) & 0xFF) as u8);
        push_back(&mut result, ((valid_until >> 16) & 0xFF) as u8);
        push_back(&mut result, ((valid_until >> 8) & 0xFF) as u8);
        push_back(&mut result, (valid_until & 0xFF) as u8);
        
        // Add cost (20 bytes = 160 bits, padded from 128 bits)
        let i = 19;
        while (i >= 0) {
            push_back(&mut result, ((cost >> (i * 8)) & 0xFF) as u8);
            if (i == 0) { break };
            i = i - 1;
        };
        
        // Add backdoor data
        append(&mut result, backdoor);
